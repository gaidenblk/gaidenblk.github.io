(function () {
	...
	function armazenaProduto(max) {
		for (let i = max; i < arrayProdutos.length; i++) {
			const dadosProdutos = [
				arrayProdutos[i].nome,
				arrayProdutos[i].preco,
				"edit",
				"delete",
			];
			const novaTr = document.createElement("tr");
			novaTr.id = arrayProdutos[i].id;
	...
	function visualizaProduto(targetId) {
		...
		produtoContainer.style.display = "flex";
	...
	btnCadastrar.addEventListener("click", () => {
		...
		armazenaProduto(arrayProdutos.length - 1);
		localStorage.setItem("arrayProdutos", JSON.stringify(arrayProdutos));
		inptNome.value = "";
		inptPreco.value = "";
		inptDescricao.value = "";
